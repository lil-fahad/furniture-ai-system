...
2. Read this file, `CLAUDE.md` when applicable, `.github/copilot-instructions.md` when applicable, and the newest issue #66 coordination records.
3. Create a dedicated task branch **at exactly the current `main` SHA**. Never start by editing `main`.
4. Run the mandatory bootstrap from the clean, unmodified task branch:
...
5. The bootstrap reads **every Git-tracked file byte-for-byte**, classifies text/binary files, counts bytes/files, and computes a deterministic repository manifest SHA-256. It also reads the live coordination board, reports the exact number of active AI sessions, what each session is doing, its branch, and its declared file scope.
6. The bootstrap must run before edits, on a clean branch whose `HEAD` equals live `main`. A dirty checkout or stale SHA is a hard failure.
7. The byte scan proves full tracked-file coverage and the trusted PR guard independently recomputes the manifest from `main`. It cannot cryptographically prove semantic understanding. The AI must still inspect the project structure and consume the tracked text/source/configuration needed to understand architecture and dependencies before changing code; large files must be processed in chunks rather than skipped.
8. If bootstrap reports a planned-scope conflict, **do not edit**. Either serialize ownership (one session waits/releases the shared scope) or establish the bilateral collaboration protocol below.
9. Only after a valid `AI-BOOTSTRAP` record exists may the session register its `AI-LEASE` and begin editing.

## AI-BOOTSTRAP receipt

A future PR is rejected unless its lease points to a matching bootstrap receipt for the same branch/session/current-main SHA. The trusted guard also recomputes the manifest and rejects forged or stale receipt values.
...
The PR guard rejects changed files outside the lease scope. If `main` advances, rebuild/update the task branch, rerun bootstrap on the new clean base, and renew the lease with the new receipt before continuing toward merge.
...
The result exposes `active_session_count` and `active_sessions`. Each session includes agent/tool label, unique session id, task, branch, base SHA, file scope, and lease expiry. Always use this live state; never copy a static agent count into documentation.

## Conflict policy: serialize or collaborate

One active session owns a file scope at a time unless a real collaboration is established. Scope conflict is checked twice: planned scope at bootstrap and exact changed-file overlap at PR time.

A unilateral `Coordination-Override` **no longer authorizes overlap**.
...
- Every PR needs an active branch-matching lease backed by a trusted-manifest bootstrap receipt.
- PR base SHA must equal current live `main`.
- Changed files must stay inside the lease scope.
- Non-draft exact-file overlap with another non-draft PR is blocked unless the bilateral collaboration + both ACKs + other-session exact-head review are present.
...
The PR coordination workflow uses `pull_request_target` and checks out trusted `main`, never PR-controlled coordination code. It recomputes the repository manifest from that trusted checkout and validates the PR's `AI-BOOTSTRAP` receipt against it.
...
Those files must direct the agent here and to the bootstrap command. This file plus issue #66 are authoritative.
