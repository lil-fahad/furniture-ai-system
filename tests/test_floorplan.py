...
def test_dark_corner_mark_does_not_turn_exterior_into_a_room() -> None:
    # Regression: floodFill used to seed at (0,0) unconditionally; a dark
    # title block there made the exterior background the largest "room".
    ...
    for room in result.rooms:
        ...
        # No room may span the whole plan (the exterior) or cover the corner mark.
...
def test_border_touching_free_space_is_not_a_room() -> None:
    # Free space connected to the image border is exterior, never a room.
...
def test_infer_room_types_falls_back_to_generic_room() -> None:
    ...
    from furniture_ai.floorplan import infer_room_types
