...
def test_professional_vision_readiness_requires_complete_bundles(tmp_path) -> None:
    root = tmp_path / "models"
    required_names = ("config.json", "model.safetensors", "preprocessor_config.json")
    for directory in ("detr_resnet50", "depth_anything_v2_small"):
        (root / directory).mkdir(parents=True)

    settings = Settings(environment="test", professional_models_root=root)
    assert settings.professional_vision_available is False

    for directory in ("detr_resnet50", "depth_anything_v2_small"):
        for name in required_names:
            (root / directory / name).write_text("test", encoding="utf-8")
    assert settings.professional_vision_available is True
