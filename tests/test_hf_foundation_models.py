...
def test_training_queue_uses_hf_convnext_baseline() -> None:
    path = Path(__file__).resolve().parents[1] / "training" / "local_training_jobs.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
    classifiers = {
        job["task"]: job
        for job in payload["jobs"]
        if job["task"] in {"style_classifier", "room_classifier"}
    }

    assert set(classifiers) == {"style_classifier", "room_classifier"}
    for job in classifiers.values():
        args = job["args"]
        model_index = args.index("--model-name") + 1
        device_index = args.index("--device") + 1
        assert args[model_index] == "hf_hub:timm/convnext_base.fb_in22k_ft_in1k"
        assert args[device_index] == "cuda"
        assert "convnext_base_v2.pth" in job["output"]
