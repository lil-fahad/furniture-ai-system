...
Room refinement uses the OpenAI Responses API with `text.format.type=json_schema` and `strict=true`. The schema is generated from the local Pydantic contract. The prompt does not duplicate the full JSON schema; schema enforcement belongs to the API contract while room-id authorization remains a local validation step.
