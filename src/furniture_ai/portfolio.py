...
class DesignPortfolioRequest(BaseModel):
    ...
    minimum_clearance: float = Field(
        default=0,
        ge=0,
        description="Clearance in the same geometry units as the floor plan.",
    )
...
class DesignPortfolioResult(BaseModel):
    ...
    selected_policy: PlacementPolicy
    execution_ready: bool
    candidates: list[DesignCandidate]
    decision_graph: DesignDecisionGraph
    ranking_basis: str = (
        "Deterministic ranking: valid layouts first; then fewer validation errors; "
        "then more placed catalog items; then fewer total issues; then request policy order. "
        "This is not an aesthetic quality score or model confidence."
    )


class DesignPortfolioEngine:
    """Generate multiple deterministic layouts and gate every candidate."""
    ...
    @staticmethod
    def _error_count(candidate: DesignCandidate) -> int:
        return sum(
            issue.severity is ConstraintSeverity.ERROR
    ...
    def _decision_graph(
        ...
        for candidate in candidates:
            relation = "selected" if candidate.id == selected.id else "ranked_below"
            builder.add_edge(f"portfolio:{candidate.id}", decision_node, relation)
