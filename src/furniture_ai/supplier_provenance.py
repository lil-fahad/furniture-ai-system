...
def authorize_supplier_row(row: Mapping[str, str]) -> SupplierProvenance:
    ...
    This is deliberately an explicit gate: legacy/catalog parsing remains
    backward-compatible, while production ingestion must opt into this check.
    No authorization is inferred from a supplier's website, claimed status,
    catalog fields, or model score.
    """
